...
- 6 条竖版场景穿搭视频示例，支持上下滚动、当前视频独占播放和点击暂停；素材统一转为 H.264/AAC。
- 首页右上角提供快捷上传入口；用户可选择本地视频/图片，或通过 `getUserMedia` 直接打开相机预览、拍照及录像，Feed 末尾继续上滑会循环回到第一条视频。
- 视频暂停后保留用户暂停时的真实画面，只显示“试试这套穿搭”入口；该暂停帧会随上身请求上传，并作为 Seedream 的场景与完整 Look 参考图。
- 评论与 AI 解析双标签，支持摘要、推荐问题和“试试这套”上身入口。
- AI 上身四步流程：拍摄人像、选择穿搭方向与身材类型、填写身高体重、授权确认和结果生成。
...
- `/admin` 可视化视频流管理面板，支持密码登录、视频上传、播放顺序调整、内置内容移除/恢复和用户上传内容永久删除。
...
管理面板位于 <http://localhost:3000/admin>。部署时需设置 `ADMIN_PANEL_PASSWORD`；未配置时管理 API 会保持关闭。用户上传媒体和管理面板的内容状态统一存放在 `USER_CONTENT_DIR`。
