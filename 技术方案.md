...
11. 通过 Next.js Route Handler 将参考图直接交给 GPT-5.6-sol，由 Sol 在同一 Responses 请求中调用 image-2。
...
- 生图：第三方兼容网关 `https://api.8989886.xyz/v1` 的 Responses 流式接口；入口模型为 `gpt-5.6-sol`，由 Sol 调用内置 `image_generation`（image-2）。
...
Nginx :443
  ...
  `-- /api/generate   -> Next.js Route Handler
                            ...
                            | 第三方网关 API Key
                            v
                   兼容网关 -> GPT-5.6-sol -> image_generation / image-2
```
...
4. 后台任务把文字、场景图和授权人像作为同一份 Responses 输入直接发给 `gpt-5.6-sol`。
5. 仅向 Sol 暴露内置 `image_generation` 工具，并在提示词中明确要求其使用 image-2、等待工具完成并返回最终图。应用代码不得再调用 `images.edit()` 或其他独立生图请求。
6. 上游请求使用 `stream: true`，持续消费 `keepalive`、生图进度和完成事件，不设置 95/110 秒一类的人为截止时间。
7. 从 `response.completed` 的 `image_generation_call.result` 读取 Base64，校验 JPEG 后写入权限为 `0600` 的任务目录；授权人像只保存在任务进程内存，不落盘。
8. 上游明确返回错误时将任务标记为 `failed`；不把场景原图作为“生成结果”回退。

单次上游请求的核心结构：

```ts
await fetch(`${IMAGE_API_BASE_URL}/responses`, {
  method: "POST",
  body: JSON.stringify({
    model: "gpt-5.6-sol",
    input: [{ role: "user", content: [prompt, sceneImage, authorizedIdentity] }],
    tools: [{
...
提示词必须把 Sol 定义为唯一编排入口，并明确图片顺序与优先级：

```text
You are the sole vision and image-generation orchestrator.
Analyze every supplied image yourself, then invoke the built-in
image_generation tool backed specifically by gpt-image-2.
Do not merely describe a prompt and do not ask the caller to invoke
another image API. Wait for the tool and return the final image.
...
该结构遵循官方建议：多图输入要按索引明确职责，构图要具体指定视角与画幅，编辑任务要区分改变项与保持项；同时 GPT-5.6 的提示词应尽量精简并避免重复规则。参考：[GPT Image prompting guide](https://developers.openai.com/cookbook/examples/multimodal/image-gen-1.5-prompting_guide#2-prompting-fundamentals)、[GPT-5.6 prompting best practices](https://developers.openai.com/api/docs/guides/model-guidance?model=gpt-5.6-sol#prompting-best-practices)。
...
- 身份板 JPEG 质量约 0.88。
- 前端同一时间只允许一个生成请求。
- 上游 Responses 使用流式 keepalive 且不设置人为生成超时；浏览器使用异步任务轮询，Nginx 无需维持数分钟的前端请求。
- `client_max_body_size` 建议 12 MB；应用层仍单独校验文件大小。
- 个性化身体与姿势使用 `quality: "medium"` 和 JPEG；相比 low 更重视比例、衣料垂坠和肢体一致性，延迟由异步任务链路承接。
...
3. 配置 Nginx 请求体和上游超时。
4. 通过服务器 `.env` 注入 `IMAGE_API_BASE_URL`、`IMAGE_API_KEY`、`VISION_ORCHESTRATOR_MODEL` 和 `AI_JOB_DIR`。
...
12. 用户确认授权后，参考图只进入 GPT-5.6-sol 请求，并由 Sol 调用 image-2 返回真实图片。
