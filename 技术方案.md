...
| 生成图成为资产 | P0 支持下载、逐版本浏览和浏览器本地收藏 | 用户账号、云端收藏、发布抖音作品 |
| 用户继续调整生成结果 | 结果页输入自然语言，以上一版本为主参考生成新照片，旧版本不覆盖 | 会话级生成树、版本存储和一致性评估 |
...
互动 H5 层
  Feed、暂停识物、搜索抽屉、评论/AI 解析、AI 上身向导
       ...
       v
生成与转化层
  人物采集、Seedream 生成、结果资产、商品与收藏/分享
```
...
11. 通过 Next.js Route Handler 将场景图、授权人像和身体数据直接交给 Seedream 5.0 Lite 生成。
...
打开网站
  ...
  -> 查看系统选定的场景和 Look 参考
  -> 选择预置人物，或拍摄正/左/右三视角
  -> 选择穿搭方向、体型、身高、体重范围和动作氛围
  -> 确认所有输入并勾选图片处理授权
  -> 点击生成
  -> Seedream 5.0 Lite 根据多参考图生成个性化画面
  -> 展示结果
  -> 输入自然语言修改场景、姿势或镜头
  -> 保留旧图并生成新版本，可在版本胶卷中切换
  -> 下载 / 逐张收藏 / 发布 / 重新设定
  -> 查看同款商品
```
...
为避免模型把任务退化成简单换脸，按固定顺序组织两类图片，并明确“保持项”和“重建项”：

```text
图片 1：场景与完整 Look 参考
  决定背景身份、服装清单、颜色、材质、配饰和光线氛围
  不决定原模特体型、姿势、肢体位置或最终裁切

图片 2：人物身份板（可选）
  包含正脸、左侧脸、右侧脸，决定身份与面部特征
  不提供身体比例

结构化属性：身高 + 体重范围 + 体型 + 穿搭方向 + 动作氛围
  身高、体重和体型共同决定肩、腰、胯、腿与纵向比例
  动作氛围决定新的头部方向、手臂、躯干、重心和腿部位置
```
...
- 穿搭方向：`男士穿搭`、`女士穿搭`。
- 身材类型：沙漏型、倒三角、梨型、直筒型；服务端转换为明确的肩/腰/胯关系。
- 身高：140～210 cm。
- 体重范围：`50 kg 以下`、`50～60 kg`、`60～70 kg`、`70～85 kg`、`85 kg 以上`。
- 动作氛围：自然抓拍、漫步动态、氛围回眸、时尚大片。
- 生成时以结构化属性重建全身可见比例，但不向用户承诺精确测量、尺码或合身度。
...
- 任务开始后允许把完整流程收起为全局状态浮窗；TryOnFlow 保持挂载并继续轮询，用户可滚动 Feed 或切换底部 Tab。
- 浮窗显示生成中、已完成或失败三种状态；点击恢复原任务页面。任务必须锁定开始生成时的 `videoId`，不能随当前 Feed 视频变化。
- 结果页以生成图为主，保留场景和 Look 缩略图用于解释输入来源。
- 提供自然语言输入框和场景、姿势、镜头快捷建议；提交后以上一张结果继续生成，不覆盖旧图。
- 版本胶卷展示当前会话的全部结果，支持切换、逐张收藏、下载和发布。
- 支持重新设定人物身体数据和返回换视频。
- 支持下载 JPEG。
- 支持 `navigator.share()` 分享文件；不支持时回退到下载提示。
- 已收藏版本以结果 URL 和描述存入浏览器本地存储，并在“我–我的穿搭”中展示；生产环境改为账号云端资产。
...
- 生图：火山方舟图片生成接口 `https://ark.cn-beijing.volces.com/api/plan/v3/images/generations`，模型为 `doubao-seedream-5.0-lite`。
- 数据：P0 使用类型安全的 Content Manifest，承载热点、搜索、AI 解析和生成配置，不使用数据库。
- 部署：Next.js standalone + systemd + Caddy，运行在 `107.175.115.136`。
...
Caddy :443
  ...
  `-- /api/generate   -> Next.js Route Handler
                            ...
                            | 火山方舟 API Key
                            v
                   Seedream 5.0 Lite 图片生成 API
```

浏览器不直接访问火山方舟。所有视频、参考图和站内接口保持同源，避免 Canvas、Cookie 和 CORS 问题。暂停识物、同款结果和 AI 解析在 P0 均为 Content Manifest 驱动，只有 AI 上身生成请求依赖外部模型服务。服务端必须显式配置完整图片生成 endpoint，不得在方舟不可用时自动回落到其他模型服务。
...
| `bodyType` | 字符串 | 沙漏、倒三角、梨型或直筒型枚举 |
| `poseStyle` | 字符串 | 自然抓拍、漫步动态、氛围回眸或时尚大片枚举 |
| `heightCm` | 数字 | 140～210 |
| `weightRange` | 字符串 | 预定义枚举 |
| `consentAccepted` | 布尔值 | 必须为 `true` |
| `revisionImage` | JPEG/PNG/WebP 文件，可选 | 继续修改时提交当前选中的上一版本，最多 8 MB |
| `revisionPrompt` | 字符串，可选 | 与 `revisionImage` 同时提交，描述场景、姿势或镜头修改，最多 300 字 |

场景参考图与 Look 参考图不由浏览器上传。服务端根据 `videoId` 从受控 Manifest 和本地文件中读取，避免客户端替换路径或提交任意服务器文件。

### 8.2 服务端处理

1. 校验请求体、枚举和授权状态，并限制 `identityBoard` 为 JPEG/PNG/WebP、最多 4 MB。
2. 根据 `videoId` 从固定安全目录读取场景与完整 Look 参考图，不接受客户端文件路径。
3. 创建 UUID 生成任务，立即返回 `202` 和 `statusUrl`；H5 每 2 秒轮询，不占用浏览器长连接。
4. 首次生成把场景图放在 `image[0]`、授权人像放在 `image[1]`；继续修改时把上一版本放在 `image[0]`、原场景/Look 放在 `image[1]`、授权人像放在 `image[2]`，连同身体数据与自然语言要求发给 `doubao-seedream-5.0-lite`。
5. 使用 `sequential_image_generation: "disabled"` 固定只生成一张图片，使用 `size: "2K"` 和 `response_format: "b64_json"` 获取可持久保存的结果。
6. 上游请求不设置 95/110 秒一类的人为截止时间；H5 继续轮询本地异步任务，直到方舟明确返回成功或失败。
7. 从 `data[0].b64_json` 读取图片；兼容方舟返回 URL 的情况并由服务器下载。校验 JPEG、PNG 或 WebP 文件签名后写入权限为 `0600` 的任务目录；授权人像只保存在任务进程内存，不落盘。
8. 上游明确返回错误时将任务标记为 `failed`；不把场景原图作为“生成结果”回退。

单次上游请求的核心结构：

```ts
await fetch(IMAGE_API_BASE_URL, {
  method: "POST",
  body: JSON.stringify({
    model: "doubao-seedream-5.0-lite",
    prompt,
    image: [sceneImage, authorizedIdentity],
    size: "2K",
...
```json
{
  "jobId": "uuid",
  "status": "queued",
  "statusUrl": "/api/generate/uuid"
}
```

查询状态：`GET /api/generate/:jobId`，状态为 `queued / processing / completed / failed`。完成后响应包含 `resultUrl`；浏览器再通过 `GET /api/generate/:jobId/result` 获取 `image/jpeg`。失败响应保留稳定错误码和用户可读信息。轮询本身没有生图倒计时；网络短暂中断时继续轮询，只有任务明确失败才停止等待。

## 9. 提示词策略

提示词直接描述 Seedream 需要完成的画面，并明确图片顺序与优先级：

服务端通过 `withQualityBaseline()` 为首次生成和每次自然语言续图统一追加质量基线，覆盖审美构图、真实人体重心与关节、自然表情和视线、手脚完整性、服装材质与受力、场景透视和一致光影。质量基线由服务端维护，不能被前端漏传或被用户的局部修改需求替代。

```text
Create one personalized, photorealistic vertical fashion photograph
from the supplied reference images.

Reference roles:
- Image 1 is authoritative for the scene identity, complete garment inventory,
  lighting atmosphere, colors, materials and accessories.
- Image 1 is not authoritative for the source model's body, exact pose,
  limb placement, head direction, subject placement or camera crop.
- Image 2, when present, is the authorized identity board.
  Preserve this adult person's recognizable face, but ignore its body.

Replace the original model in Image 1 with the person from Image 2.
Reconstruct the neck-down body from height, weight range and the selected
shoulder/waist/hip relationship. Adapt garment fit and drape to this body.
Use the selected new pose. It must differ from Image 1 in head direction,
arms, torso angle, weight distribution and leg position.
Use a full-body or head-to-calf frame so the personalized silhouette is visible.
Show one adult person only. Keep anatomy natural.
Do not add text, logos, watermarks, extra garments, or unrelated accessories.
```

未上传独立人像时，提示词改为保留 Image 1 中人物的脸；不得虚构不存在的第二张图。无论是否上传人像，都必须重新构建颈部以下身体与姿势，不能沿用原模特轮廓。提示词使用“目标 → 参考职责 → 身体重建 → 姿势与镜头 → 保持与排除”的短分段结构，避免重复约束。

该结构遵循 Seedream 官方建议：多图输入要明确每张图的职责，图像编辑任务要明确改变项与保持项，并使用简洁、连贯的自然语言描述主体、动作、环境、光影与构图。参考：[Seedream 4.0–5.0 提示词指南](https://www.volcengine.com/docs/82379/1829186)。

模型仍可能改变人物细节、服装纹理或背景构图，因此必须提前用全部预置视频和人物组合做样例测试，而不是只验证 API 能返回图片。
...
- 身份板 JPEG 质量约 0.88。
- 前端同一时间只允许一个生成请求。
- 方舟图片生成请求不设置人为生成超时；浏览器使用异步任务轮询，Caddy 无需维持数分钟的前端请求。
- Caddy 请求体上限建议 12 MB；应用层仍单独校验文件大小。
- 个性化身体与姿势使用 2K 单图模式；延迟由异步任务链路承接。
...
| 浏览器轮询时网络中断 | 保持任务运行并继续轮询，不因临时断网结束生图 |
| 第三方网关明确返回 4xx/5xx | 将任务标记失败并展示真实状态；不伪造结果、不回落到 OpenAI 官方域名 |
| 分享 API 不支持 | 回退为下载文件 |
...
- 拍摄前说明用途，提交前再次获得明确授权。
- 文案必须说明图片会发送至火山方舟 Seedream 图像模型处理，并标明服务提供方。
- 应用自身不写数据库、不长期保存人脸图、不上传到其他存储。
- 不在日志、错误追踪或分析事件中记录图片内容。
- 关闭页面、刷新页面或替换人物后释放当前 Blob 和 Object URL。
- 摄像头拍摄完成或退出流程时立即停止媒体轨道。
- API Key 仅保存在服务器环境变量中，使用 `IMAGE_API_KEY`；完整图片生成地址使用 `IMAGE_API_BASE_URL`，模型名使用 `IMAGE_API_MODEL`。
- 前端不得出现任何 `NEXT_PUBLIC_*` API Key。
- 管理和部署端口不暴露到公网，只开放 22、80、443。
- 生成接口加入基础 IP 限流和并发锁，避免演示链接被刷接口。

上线前必须根据火山方舟的最新服务条款确认图片、提示词和日志的保留周期、训练用途、删除机制及地域边界；确认前不得向用户承诺“不留存”，也不应在公开演示中采集无授权的真实人脸。

## 13. VPS 部署方案

部署目标：`107.175.115.136`，生产域名为 `dy.fangwenky.dpdns.org`。

### 13.1 访问结构

```text
https://dy.fangwenky.dpdns.org/
  -> Caddy :443
     -> Next.js :3200
```
...
```text
systemd
  `- scene-fit-dy    Next.js standalone
...
1. 将 `dy.fangwenky.dpdns.org` 指向 `107.175.115.136`。
2. 由 Caddy 自动获取并续期可信 HTTPS 证书。
3. 配置 Caddy 请求体和反向代理规则。
4. 通过服务器 `.env` 注入 `IMAGE_API_BASE_URL`、`IMAGE_API_KEY`、`IMAGE_API_MODEL`、`IMAGE_API_SIZE` 和 `AI_JOB_DIR`。
5. 创建非 root 部署用户；确认密钥登录后再关闭 root 密码登录。
6. 防火墙只开放 22、80、443。
7. systemd 服务设置健康检查和自动重启。
...
12. 用户确认授权后，参考图只由服务器提交给 Seedream 5.0 Lite，并返回真实生成图片。
13. 用户可以用自然语言要求更换场景、姿势或镜头，模型基于选中版本生成新照片且保留旧版本。
14. 用户可以在版本胶卷中查看全部结果，并将任意版本单独收藏、下载或发布。
15. 用户可以从结果页打开与当前 Look 对应的商品视图。
16. 摄像头拒绝、内容审核、限流、断网和 API 失败不会让页面卡死；无明确上游失败时持续等待。
17. 目标手机通过正式 HTTPS 地址可以完成三轮完整链路。
18. 所有预置视频 × 预置人物组合都至少提前生成并人工检查一次。
...
4. 36～40 小时：完成 standalone、systemd、Caddy、域名与 HTTPS 部署。
