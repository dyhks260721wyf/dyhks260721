...
Reference roles:
- Image 1 is authoritative for the scene, complete outfit, pose,
  composition, lighting, atmosphere, colors and accessories.
- Image 2, when present, is the authorized identity board.
  Preserve this adult person's recognizable facial identity.

Replace the original model in Image 1 with the person from Image 2.
Keep the scene and complete outfit from Image 1.
...
未上传独立人像时，提示词改为保留 Image 1 中人物；不得虚构不存在的第二张图。地点、内容摘要和身材信息只能补充氛围与大致比例，不允许覆盖参考图职责，也不允许把下游生图调用重新交还给应用服务。
